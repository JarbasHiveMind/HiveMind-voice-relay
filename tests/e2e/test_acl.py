"""ACL policy-admission e2e tests for HiveMind-voice-relay.

Three enforcement paths from the PolicyChain model
...
3. session_id="default" denied for non-admin — a non-admin satellite that
   ...
   INTENTIONAL xfail (strict=False): the voice-relay bridge's
   _install_client_session rewrites the session before the policy runs in
   e2e dispatch — defence-in-depth by design.  SESSION_ID_DEFAULT_FORBIDDEN
   coverage lives in ovos-agent-plugin unit tests, not here.
"""
...
@pytest.mark.xfail(
    strict=False,
    reason=(
        "INTENTIONAL defence-in-depth: the voice-relay bridge's "
        "_install_client_session rewrites the session before the policy runs "
        "in e2e dispatch, so SESSION_ID_DEFAULT_FORBIDDEN is not observable "
        "at this layer.  Coverage lives in ovos-agent-plugin unit tests."
    ),
)
def test_default_session_id_denied_for_non_admin():
    ...
    try:
        m = b.get_master("M0")
        # Use S_RESTRICTED_SKILL: utterance type is allowed, so
