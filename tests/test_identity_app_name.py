...
def test_the_cli_asks_for_its_own_identity_and_hands_it_to_the_client():
    from hivemind_voice_relay import __main__ as entry
...
def test_the_service_asks_for_its_own_identity_and_hands_it_to_the_client():
    from hivemind_voice_relay import service
    identity = _identity()
    with patch.object(service, "NodeIdentity", return_value=identity) as ctor, \
         patch.object(service, "HiveMessageBusClient") as client:
        bus = service.get_bus()
    ctor.assert_called_once_with(app_name=APP_NAME)
    assert bus is client.return_value
    assert client.call_args.kwargs.get("identity") is identity, (
        "the client was built without the application's identity")
